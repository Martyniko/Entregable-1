...
public class FXMLCursoViewController implements Initializable {
    ...
    private void iniDias(){
        ...
        diasImparte.forEach((item)-> {item.onProperty().addListener((obs, wasOn, isNowOn) -> {
//            if (isNowOn) nDias.add(Dias.valueOf(item.getName()));
//            else nDias.remove(Dias.valueOf(item.getName()));
